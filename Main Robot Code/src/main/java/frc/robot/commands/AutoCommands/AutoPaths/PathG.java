...
public class PathG extends SequentialCommandGroup {
  public PathG(DriveTrainSubsystem driveTrainSubsystem, BallSubsystem ballSubsystem) {
    // place to Start
    super(
      new TurnToAngle(driveTrainSubsystem, Robot.shuffleBoard.PathGAngle1.getDouble(0), Wheel.left),
      new DriveDistance(driveTrainSubsystem, Robot.shuffleBoard.PathGDistance2.getDouble(0), Robot.shuffleBoard.PathGSpeed2.getDouble(0)),
      // new AccelerateDrive(driveTrainSubsystem, 0, -.25, Wheel.both)
      new TurnToAngle(driveTrainSubsystem, Robot.shuffleBoard.PathGAngle3.getDouble(0), Wheel.left),
